...
import { Package, DollarSign, Search, X, Wallet, MapPin } from 'lucide-react'
import { usePayableDangoteOrders, usePayDangoteOrder } from '#/lib/hooks/useDangoteOrders'
import { PageLoader } from '#/components/PageLoader'
import { PageError } from '#/components/PageError'
import { PageEmpty } from '#/components/PageEmpty'
import { Pagination } from '#/components/Pagination'

export const Route = createFileRoute('/dangote-payable-orders/')({
  component: PayableDangoteOrdersPage,
})
...
function PayableDangoteOrdersPage() {
  ...
  const payOrder = usePayDangoteOrder()
  ...
  if (isLoading) return <PageLoader message="Loading payable Dangote orders..." />
  if (isError) return <PageError message={(error as any)?.message || 'Failed to load payable orders'} onRetry={refetch} />

  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-xl md:text-2xl font-semibold text-foreground tracking-tight text-balance">Payable Dangote Orders</h1>
        <p className="text-muted-foreground">
          Approved Dangote orders where the customer's wallet balance covers the total.
      ...
      <Card>
        <CardHeader className="border-b border-border">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <CardTitle>Payable Dangote Order Register</CardTitle>
              <CardDescription>Approved unpaid Dangote orders with sufficient customer wallet balance</CardDescription>
            </div>
        ...
        <CardContent>
          {filteredOrders.length === 0 ? (
            <PageEmpty
              ...
              title="No payable Dangote orders"
              description={searchTerm
                ? 'No orders match your search.'
                : 'There are no approved unpaid Dangote orders where the customer has sufficient wallet balance.'}
                    ...
                    {paginatedOrders.map((order: any) => {
                      ...
                      return (
                        <TableRow
                          ...
                          onClick={() => navigate({ to: '/dangote-orders/details' as any, search: { id: String(order.id) } as any })}
                        >
