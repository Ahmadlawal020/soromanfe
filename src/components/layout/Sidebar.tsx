      ...
      {items.map((item) => {
        ...
        return (
          ...
          >
            ...
            {expanded && <span className="min-w-0 flex-1 truncate font-medium">{item.title}</span>}
    ...
    <>
      ...
      >
        ...
        <div className="flex h-16 items-center justify-between px-4 border-b border-white/10 shrink-0">
          <div
            className={cn('flex items-center gap-3 min-w-0', !expanded && 'justify-center flex-1')}
            title={expanded ? undefined : 'Soroman'}
          >
            <img src="/logo.png" alt="Soroman" className={cn("shrink-0", expanded ? "w-9 h-9" : "w-8 h-8")} />
            {expanded && (
              <div className="min-w-0 leading-tight">
                <div className="font-semibold text-sm tracking-tight text-white">Soroman</div>
          ...
          {expanded ? (
            <div className="mb-3 flex items-center gap-3 px-2 rounded-lg py-1.5">
              ...
              <div className="min-w-0">
                ...
                <div className="truncate text-xs text-white/60">{user?.email || 'admin'}</div>
              </div>
            </div>
          ) : (
