...
interface LpgCompletionStepProps {
  wizard: LpgOrderWizardReturn
}

export function LpgCompletionStep({ wizard }: LpgCompletionStepProps) {
  ...
  return (
    <div key="lpg-step-6" className="space-y-6 animate-fade-in">
      {/* Success Header */}
      ...
      <div className="max-w-2xl mx-auto border rounded-xl divide-y divide-border">
        ...
        <div className="p-4 grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
          ...
          <div>
            <span className="text-xs text-muted-foreground block">Cylinder Size</span>
            <span className="font-semibold text-foreground">{placedRequest.cylinderSizeKg} Kg</span>
          </div>
          <div>
            <span className="text-xs text-muted-foreground block">Quantity</span>
            <span className="font-semibold text-foreground">{Number(placedRequest.cylinderQuantity).toLocaleString()} cylinders</span>
          </div>
          <div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-center pt-2 pb-4">
        ...
        <Button onClick={() => navigate({ to: '/lpg-order-request' as any })}>
          View LPG Requests <ArrowRight className="size-4 ml-2" />
        </Button>
      </div>
    </div>
  )
